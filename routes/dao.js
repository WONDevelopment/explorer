...
var daoContract = won.contract(daoABI);
...
module.exports = function(req, res){
  ...
  if (!("action" in req.body))
    ...
    try {
      ...
      var extraBalance = won.getBalance(extraBalanceAddr);
      extraBalance = wonUnits.toWon(extraBalance, 'wei');
    ...
    ctFind.exec(function (err, docs) {
      if (!docs.length){
        ...
        var formatDocs = docs.map( function(doc) {
          var d = doc;
          d.amount = wonUnits.toWon(d.amount, 'wei')*100;
          return d;
        });
